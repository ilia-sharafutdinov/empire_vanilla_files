...
local enable_jihadist_diplomacy = false -- Set to 'true' to enable diplomacy while playing jihadists. Not recommended - may cause instability
local test_as_plains = false -- For testing purposes. Set to 'true' to make automated playtests using Plains Nations

-- VARIABLES
morocco_is_human = false
georgia_is_human = false
...
local function OnFactionTurnStart(context)
	if enable_missions then
		if conditions.TurnNumber(context) == 2 then
			if conditions.FactionName("britain", context) and conditions.FactionIsHuman("britain", context) then
				scripting.game_interface:trigger_custom_mission("make_alliance_maratha", "britain", "forge_alliance", 0, "maratha", "", "", "", 1000, "", context)
				scripting.game_interface:trigger_custom_mission("capture_sindh", "britain", "capture_city", 0, "sindh", "", "", "", 1500, "", context)
			elseif conditions.FactionName("british_company", context) and conditions.FactionIsHuman("british_company", context) then
				scripting.game_interface:trigger_custom_mission("capture_carnatica", "british_company", "capture_city", 0, "carnatica", "", "", "", 1000, "", context)
			end
		elseif conditions.TurnNumber(context) == 3 then
			if conditions.FactionName("britain", context) and conditions.FactionIsHuman("britain", context) then
				scripting.game_interface:trigger_custom_mission("canada_protectorate", "britain", "protectorate_region_capture", 0, "great_plains+algonquin_territory+new_england", "", "", "", 0, "canada", context)
			elseif conditions.FactionName("spain", context) and conditions.FactionIsHuman("spain", context) then
				scripting.game_interface:trigger_custom_mission("new_spain_protectorate", "spain", "protectorate_region_capture", 0, "jamaica+curacao", "", "", "", 0, "new_spain", context)
			elseif conditions.FactionName("revolutionary_france", context) and conditions.FactionIsHuman("revolutionary_france", context) then
				scripting.game_interface:trigger_custom_mission("morocco_protectorate", "revolutionary_france", "protectorate_region_capture", 0, "algiers+gibraltar+malta", "", "", "", 0, "morocco", context)
			elseif conditions.FactionName("russia", context) and conditions.FactionIsHuman("russia", context) then
				scripting.game_interface:trigger_custom_mission("georgia_protectorate", "russia", "protectorate_region_capture", 0, "chechenya-dagestan", "", "", "", 0, "georgia", context)
			end
	...
	if conditions.FactionName("morocco", context) and conditions.FactionIsHuman("morocco", context) then
		morocco_is_human = true
	elseif conditions.FactionName("georgia", context) and conditions.FactionIsHuman("georgia", context) then
		georgia_is_human = true
	end

	if handover_protectorates then
		if conditions.FactionName("revolutionary_france", context) and (not conditions.FactionIsHuman("revolutionary_france", context) or not enable_missions) and not morocco_is_human then
			scripting.game_interface:grant_faction_handover("revolutionary_france", "morocco", 6, 15, context)
		elseif conditions.FactionName("russia", context) and (not conditions.FactionIsHuman("russia", context) or not enable_missions) and not georgia_is_human then
			scripting.game_interface:grant_faction_handover("russia", "georgia", 5, 10, context)
		end
	end
end
...
local function OnWorldCreated()
	...
	if not enable_jihadist_diplomacy then
		for key, faction in ipairs(all_factions_list) do
			if faction ~= "jihadists" then
				scripting.game_interface:force_diplomacy("jihadists", faction, "peace", false, false)
				scripting.game_interface:force_diplomacy(faction, "jihadists", "peace", false, false)
			end
		end
	end
