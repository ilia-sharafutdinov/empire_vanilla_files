...
def change_regions_slot_description_coordinates(soup, base_coordinates, n_slots, slot_n):
    x_new = str(base_coordinates[0] + coordinates_deltas[n_slots][slot_n][0])
    y_new = str(base_coordinates[1] + coordinates_deltas[n_slots][slot_n][1])
    soup.find('rec', {'type': 'slot_descriptions'}).find_all('v2')[0]['x'] = x_new
    soup.find('rec', {'type': 'slot_descriptions'}).find_all('v2')[1]['x'] = x_new
    soup.find('rec', {'type': 'slot_descriptions'}).find_all('v2')[0]['y'] = y_new
    soup.find('rec', {'type': 'slot_descriptions'}).find_all('v2')[1]['y'] = y_new
    return soup
...
examples:
  ...
  Turning Brussels into a major 5-slot city
    ...
    python add_building_slot.py -u -g magistrate -b army_encampment flanders settlement_army-adminflanders
    python add_building_slot.py -b cannon_foundry flanders settlement_ordnanceflanders
    python add_building_slot.py -b conservatorium flanders settlement_cultureflanders
    python add_building_slot.py -b admiralty flanders settlement_navy-admin
''')
...
controlling_faction_id = region_soup.find('rec', {'type': 'REGION'}).find_all('u')[9].string

settlement = region_soup.find('rec', {'type': 'REGION'}).find_all('xml_include', recursive=False)[1].get('path').replace(f'region_slot/settlement-{region}-', '').replace('.xml', '')

region_slot_array = region_soup.find('rec', {'type': 'REGION'}).find('rec', {'type': 'REGION_SLOT_MANAGER'}).find('ary', {'type': 'REGION_SLOT_ARRAY'})
region_slots = [i for i in region_slot_array.find_all('xml_include') if 'region_slot/settlement' in i.get('path')]
...
cai_world_building_slots = cai_world_slot_array.find_all('xml_include')
...
# Changing BDI
new_cai_building_slot_soup.find('rec', {'type': 'CAI_WORLD_BUILDING_SLOTS'}).find_all('u4_ary', recursive=False)[1].string = ''
if keep_bdi:
    new_cai_building_slot_soup.find('rec', {'type': 'CAI_WORLD_BUILDING_SLOTS'}).find_all('u4_ary', recursive=False)[3].string = \
    new_cai_building_slot_soup.find('rec', {'type': 'CAI_WORLD_BUILDING_SLOTS'}).find_all('u4_ary', recursive=False)[3].string.split(' ')[-1]
else:
    new_cai_building_slot_soup.find('rec', {'type': 'CAI_WORLD_BUILDING_SLOTS'}).find_all('u4_ary', recursive=False)[3].string = ''
new_cai_building_slot_soup.find('rec', {'type': 'CAI_WORLD_BUILDING_SLOTS'}).find_all('u4_ary', recursive=False)[7].string = ''
...
settlement_id = settlement_soup.find('rec', {'type': 'SETTLEMENT'}).find_all('i', recursive=False)[1].string
...
if upgrade_minor_to_major and clear_government_slot_bdi:
    ...
    # Changing BDI
    government_cai_building_slot_soup.find('rec', {'type': 'CAI_WORLD_BUILDING_SLOTS'}).find_all('u4_ary', recursive=False)[1].string = ''
    if keep_bdi:
        government_cai_building_slot_soup.find('rec', {'type': 'CAI_WORLD_BUILDING_SLOTS'}).find_all('u4_ary', recursive=False)[3].string = \
        government_cai_building_slot_soup.find('rec', {'type': 'CAI_WORLD_BUILDING_SLOTS'}).find_all('u4_ary', recursive=False)[3].string.split(' ')[-1]
    else:
        government_cai_building_slot_soup.find('rec', {'type': 'CAI_WORLD_BUILDING_SLOTS'}).find_all('u4_ary', recursive=False)[3].string = ''
    government_cai_building_slot_soup.find('rec', {'type': 'CAI_WORLD_BUILDING_SLOTS'}).find_all('u4_ary', recursive=False)[7].string = ''
...
regions1_slots = [i for i in regions1_slot_array.find_all('xml_include') if 'slot/settlement' in i.get('path') and not 'fortification' in i.get('path') and not 'road' in i.get('path')]
...
new_regions1_slot_soup.find('rec', {'type': 'slot_descriptions'}).find_all('s', recursive=False)[1].string = new_slot
...
if upgrade_minor_to_major:
    ...
    government_regions1_slot_soup.find('rec', {'type': 'slot_descriptions'}).find_all('s', recursive=False)[1].string = 'settlement_government'
