import argparse
import os
import pandas as pd

from ..lib import *



def change_faction_majority(faction_name: VANILLA_FACTION_KEY_HINTS, majority: Literal['yes', 'no']):
    '''
    example:
        change_faction_majority('punjab', 'yes')
    '''

    assert faction_name in FACTION_IDS['name'].to_list(), f"{faction_name} is not present in {FACTION_IDS_PATH}."

    # Collecting info

    faction_info = FACTION_IDS[FACTION_IDS['name'].eq(faction_name)].iloc[0]

    # Reassigning faction majority

    faction = Faction(os.path.join(OUTPUT_DIR, 'factions', faction_info['path']))

    if majority == 'yes':
        faction.make_major()
    else:
        faction.make_minor()

    # Reassigning faction majority in preopen_map_info

    preopen_map_info = PreopenMapInfo(os.path.join(OUTPUT_DIR, 'preopen_map_info', Esf(os.path.join(OUTPUT_DIR, 'esf.xml')).get_preopen_map_info()))

    if majority == 'yes':
        preopen_map_info.get_faction_info(faction_info['name']).make_major()
    else:
        preopen_map_info.get_faction_info(faction_info['name']).make_minor()

    # Applying changes

    faction.write_xml()

    preopen_map_info.write_xml()



if __name__ == '__main__':

    parser = argparse.ArgumentParser(
        formatter_class=argparse.RawTextHelpFormatter,
        description='',
        epilog='example:\n' +
        '  python change_faction_majority.py punjab yes')
    parser.add_argument('faction_name', type=str, help='')
    parser.add_argument('majority', type=str, choices=['yes', 'no'], help='')
    args = parser.parse_args()

    faction_name = args.faction_name
    majority = args.majority

    FACTION_IDS = pd.read_csv(FACTION_IDS_PATH, delimiter='\t', dtype=str)

    change_faction_majority(faction_name, majority)
